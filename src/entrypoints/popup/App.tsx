...
function App() {
  ...
  // Save state when psm or selectedVars changes
  useEffect(() => {
    if (!settings) return;

    const timer = setTimeout(() => {
      storage.saveLastState(psm, selectedVars);
    }, 500);

    return () => clearTimeout(timer);
  }, [psm, selectedVars, settings]);
  ...
  const loadSettings = async () => {
    ...
    if (data.lastState) {
      setPsm(data.lastState.psm || '');
      // Merge defaults with saved vars, saved vars take precedence
      setSelectedVars({ ...defaults, ...data.lastState.selectedVars });
    } else {
  ...
  return (
    <div className="w-[560px] min-h-[300px] bg-gray-50 p-4 font-sans text-gray-900">
                ...
                <div className="flex flex-wrap gap-2 mt-2 pt-2 border-t border-gray-100">
                  {platformVars.map(v => (
                    <div key={v.id} className="flex items-start gap-2 text-xs">
                      ...
                      <div className="flex flex-wrap bg-gray-100 rounded p-0.5 w-full">
                        {v.values.map(val => (
                          <button
                            key={val.value}
                            onClick={(e) => {
                            ...
                            className={`px-2 py-0.5 rounded transition-colors ${selectedVars[v.name] === val.value
